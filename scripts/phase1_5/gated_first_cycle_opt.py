#!/usr/bin/env python3
"""ABLATION (non-destructive): "GATED first-cycle optimization (db=1.0 only when cycle-0 optimize K>=300)".
...
Saves to FROZEN_GATED_FIRST_CYCLE_OPT/. Does NOT touch the frozen Tier A artifacts."""
...
SUB = OUT / "FROZEN_GATED_FIRST_CYCLE_OPT"; SUB.mkdir(parents=True, exist_ok=True)
...
def run(topo, force_first):
    ...
    for i, t in enumerate(range(lo, hi)):
        ...
        dbb = DB_STEADY
        kind, K, _ = ACTIONS[a]
        if force_first and i == 0 and kind != "keep" and K >= 300:   # GATED: only large-K (hard) cold starts
            dbb = 1.0                                                 # small-K & KEEP topos stay frozen
        if kind == "keep":
...
for topo in WIN:
    ...
    b["variant"]="baseline_repro"; f["variant"]="gated_first_cycle_opt"; base_all.append(b); abl_all.append(f)
...
pd.concat(abl_all).to_csv(SUB/"gated_first_cycle_opt_per_cycle.csv", index=False)
