...
para("On all eight evaluated topologies the learned controller achieves PR ≥ 0.90 with mean and p95 "
     ...
     "K, no RandomForest, and no full-OD optimization. Over the five source-locked FlexDATE topologies (Abilene, "
     "CERNET, GEANT, Sprintlink, Tiscali) it achieves 3/5 learned FlexDATE wins (Abilene, CERNET, GEANT). "
     "Sprintlink reaches a learned PR of 0.9960 and Tiscali 0.9522, both below the strict 0.999 PR target, so "
     "neither is a learned FlexDATE win (both meet their DB targets). Separately, a deployable bottleneck-ranking "
     "K800 route reaches Sprintlink PR 0.9993 under 500 ms (4/5 on the deployable track); this is a diagnostic "
     "result and is not claimed as learned-policy output. Tiscali's PR gap to 0.999 is not closed even by forced "
     "large-K, so Tiscali is a genuine loss on both tracks.")
...
flex_rows=[
 ["Abilene","learned DDQN","0.958","0.9843","0.0513","0.0058","11.6","20.4","Yes"],
 ["CERNET","learned DDQN","0.975","0.9925","0.0183","0.0002","56.2","120.7","Yes"],
 ["GEANT","learned DDQN","0.995","0.9983","0.0296","0.0030","66.6","121.1","Yes"],
 ["Sprintlink","learned DDQN","0.999","0.9960","0.0510","0.0034","179.6","278.8","No"],
 ["Tiscali","learned DDQN","0.999","0.9522","0.0510","0.0020","99.3","298.4","No"],
...
para("Over the five source-locked FlexDATE topologies (Abilene, CERNET, GEANT, Sprintlink, Tiscali), the learned "
     "DDQN achieves 3/5 FlexDATE wins (Abilene, CERNET, GEANT). Sprintlink (learned PR 0.9960) and Tiscali "
     "(learned PR 0.9522) miss the strict 0.999 PR target; both meet their DB targets. With the deployable "
     "bottleneck-ranking K800 route, Sprintlink also reaches PR 0.9993 under 500 ms, giving 4/5 on the deployable "
     "track. Tiscali's PR gap to 0.999 is large and is NOT closed even by forced large-K (its worst-case-hardened "
     "mean is ~0.975), so Tiscali is a genuine loss on both tracks. The Sprintlink deployable rows are "
     "search/actuator-verified, not learned-policy output.", bold=True, size=9.5)
h2("Claim boundary")
para("A. Final learned runtime-safe result. Over the five source-locked FlexDATE topologies, the learned DDQN "
     "achieves 3/5 learned FlexDATE wins (Abilene, CERNET, GEANT). Sprintlink learned PR = 0.9960 < 0.999 and "
     "Tiscali learned PR = 0.9522 < 0.999 (both meet the DB target), so neither is a learned FlexDATE win. "
     "Tiscali's PR gap is not closed even by forced large-K, so it is a genuine loss; Sprintlink's 0.999 IS "
     "reachable via the deployable route (Part B), giving 4/5 on the deployable track.")
...
code("Final learned runtime-safe result:\n"
     ...
     "K. Over the five source-locked FlexDATE topologies (Abilene, CERNET, GEANT, Sprintlink, Tiscali) it\n"
     "achieves 3/5 learned FlexDATE wins (Abilene, CERNET, GEANT). Sprintlink (0.9960) and Tiscali (0.9522)\n"
     "miss the 0.999 PR target as learned policy; both meet their DB targets.\n\n"
     "High-accuracy Sprintlink diagnostic:\n"
     "A deployable bottleneck-ranking K800 route achieves Sprintlink PR>=0.999 under 500 ms (4/5 on the\n"
     "deployable track), but the learned DDQN did not select it reliably enough to claim a learned Sprintlink win.\n"
     "Tiscali's PR gap to 0.999 is not closed even by forced large-K, so Tiscali is a genuine loss on both tracks.")
