...
SUB = OUT / "FULLDATA_GATED_PRESERVED"; SUB.mkdir(parents=True, exist_ok=True)
...
EPOCHS, LR, EPS, LAMBDA_DISTILL = 4, 3e-4, 0.10, 2.0     # conservative fine-tune + behavior-preservation
...
if __name__=="__main__":
    ...
    # checkpoint selection: rank by action-match with frozen policy (cheap), then full-eval the best
    sel=[]
    for ck in ckpts:
        am=action_match(ck, teacher); sel.append((am,ck)); print(f"  [select] {ck.name} action_match={am:.4f}",flush=True)
    sel.sort(reverse=True); pd.DataFrame([{"checkpoint":c.name,"action_match":a} for a,c in sel]).to_csv(SUB/"validation_selection_log.csv",index=False)
    best_am, best_ck = sel[0]; print(f"\n[selected] {best_ck.name} (action_match={best_am:.4f})",flush=True)
    ...
    pc=gated_eval(bm, vtl_n=40); pc.to_csv(SUB/"fulldata_gated_eval_per_cycle.csv",index=False)
