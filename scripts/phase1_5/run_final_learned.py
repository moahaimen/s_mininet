...
SUB = OUT / "FINAL_LEARNED_4OF5_BOTTLENECK_RANKING_DDQN"; SUB.mkdir(parents=True, exist_ok=True)
...
def part_a():
    ...
    for tag, (pkey, ilo, ihi, is_train) in jobs.items():
        ...
        for t in range(ilo, ihi):
            ...
            if is_train:
                for K in A.K_LIST if hasattr(A, "K_LIST") else [50,100,200,300,500,800]:
                    sel = ranked[:K]; s0 = time.perf_counter()
                    lp = solve_selected_path_lp_dbbudget(tm_vector=tm, selected_ods=sel, base_splits=ecmp,
                        path_library=pl, capacities=caps, prev_splits=ecmp, db_budget=1.0, db_weight=1e-6, time_limit_sec=60)
                    ms = (time.perf_counter()-s0)*1000 + GNN_MS[topo]
                    so = np.array(sorted(int(o) for o in sel if tm[o] > 0), np.int32)
                    ss = [np.asarray(lp.splits[int(o)], np.float32) for o in so]
                    opt[(t, K)] = dict(mlu=float(lp.routing.mlu), ms=float(ms), sel_ods=so, sel_splits=ss)
...
def reward(PR, mlu_ex, DB, ms, k, nact, is_keep, tgt):
    ...
    if PR >= tgt: r += BONUS
        ...
        if is_keep and PR < 0.90: r -= KEEP_GATE*(0.90 - PR)
...
def part_b():
    ...
    torch.save({"state_dict": online.state_dict(), "dim": dim, "n_act": 7, "controller_type": "Double-DQN",
                "topology_agnostic": True, "bottleneck_ranking": True}, SUB / "final_learned_4of5_model.pt")
    pd.DataFrame(tlog).to_csv(SUB / "final_learned_4of5_train_log.csv", index=False)
...
def part_c():
    dim = len(A.AGN_FEAT_NAMES); ck = torch.load(SUB / "final_learned_4of5_model.pt", map_location="cpu")
    ...
    for topo in TOP:
        ...
        for t in range(lo, hi):
            ...
            if kind == "keep": mlu = keep_mlu; ms = 0.5; k = 0; sp = accepted; non = cur_non
                ...
                sel = list(rankings[t][:K]); s0 = time.perf_counter()
                lp = solve_selected_path_lp_dbbudget(tm_vector=tm, selected_ods=sel, base_splits=ecmp,
                    path_library=pl, capacities=caps, prev_splits=accepted, db_budget=0.051, db_weight=1e-6, time_limit_sec=120)
                mlu = float(lp.routing.mlu); ms = (time.perf_counter()-s0)*1000 + GNN_MS[topo]
                k = int(len([o for o in sel if tm[o] > 0])); sp = lp.splits; non = k
    ...
    pc = pd.concat(pcs, ignore_index=True); pc.to_csv(SUB / "final_learned_4of5_eval_per_cycle.csv", index=False)
    return pc
...
if __name__ == "__main__":
    ...
    s = pd.DataFrame(srows); s.to_csv(SUB / "final_learned_4of5_summary.csv", index=False)
    pd.DataFrame(wide).to_csv(SUB / "final_learned_4of5_action_distribution.csv", index=False)
    # FlexDATE table
    fr = []
    ...
    flex = pd.DataFrame(fr); flex.to_csv(SUB / "final_learned_4of5_flexdate_table.csv", index=False)
    # audit
    ...
    audit = {"controller_type": "Double-DQN", "topology_one_hot_used": False, "topology_id_used": False,
        "bottleneck_ranking_used": True, "ranking_uses_oracle": False, "ranking_uses_pathopt": False,
    ...
    json.dump(audit, open(SUB / "FINAL_LEARNED_4OF5_AUDIT.json", "w"), indent=2)
    (SUB / "FINAL_LEARNED_4OF5_AUDIT.md").write_text("# Final Learned 4/5 — Audit\n\n" +
    ...
    (SUB / "FINAL_LEARNED_4OF5_VERDICT.md").write_text("\n".join(V))
