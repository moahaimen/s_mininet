...
SUB = OUT / "FINAL_LEARNED_4OF5_ITER2_DDQN"; SUB.mkdir(parents=True, exist_ok=True)
CACHE = OUT / "FINAL_LEARNED_4OF5_KPATH4_DDQN" / "_cache"   # REUSE kpath4 rankings + optimize tables
...
BONUS, TARGET_GATE, KEEP_GATE = 10.0, 25.0, 25.0   # Fix 4: stronger bonus/gate -> bias K toward target
KEEP_FLAT = 4.0
MS_GATE = 25.0                                       # Fix 3: steeper over-budget penalty
GAMMA, BATCH, BUFCAP, WARMUP, TUPD, EPISODES = 0.5, 128, 50000, 500, 500, 22
EPS0, EPS1, EPSDECAY = 1.0, 0.05, 16000
def reward(PR, mlu_ex, DB, ms, k, nact, is_keep, tgt, opt_feasible):
    # opt_feasible = some optimize action reaches the target with ms<500 at this cycle.
    r = W_PR*PR - W_MLU*mlu_ex - W_DB*DB - W_MS*ms - W_K*(k/max(nact,1))
    if PR >= tgt: r += BONUS
    else:
        if is_keep:
            if opt_feasible:                     # Fix 3: only punish KEEP if a sub-500ms optimize could reach target
                r -= TARGET_GATE*(tgt - PR) + KEEP_FLAT
                if PR < 0.90: r -= KEEP_GATE*(0.90 - PR)
            # else: optimizing would blow the budget -> KEEP is the correct choice, no penalty
        else:
...
def part_b():
    ...
    for topo in SEEN:
        ...
        tgt = target_pr(topo)
        # feasibility: at cycle t, does some optimize action reach the target with ms<500?
        feasible = {}
        for t in range(lo, min(hi, lo+TRAIN_CAP)):
            feasible[t] = any(pr_of(d["opt"][t], otab[(t, K)]["mlu"]) >= tgt and otab[(t, K)]["ms"] < 500.0
                              for K in [50, 100, 200, 300, 500, 800])
        CTX[topo] = dict(d=d, caps=caps, ds=ctx["ds"], pl=ctx["pl"], ecmp=ctx["ecmp"], raws=raws, otab=otab,
                         lo=lo, hi=min(hi, lo+TRAIN_CAP), tgt=tgt, feasible=feasible)
    def recon(ecmp, e):
        f = clone_splits(ecmp)
    ...
    for ep in range(EPISODES):
        ...
        for topo in order:
            c = CTX[topo]; d, caps, pl, ecmp, ds, raws, otab, lo, hi, tgt, feasible = (c["d"], c["caps"], c["pl"], c["ecmp"], c["ds"], c["raws"], c["otab"], c["lo"], c["hi"], c["tgt"], c["feasible"])
            ...
            for t in range(lo, hi):
                ...
                r = reward(PR, mex, DB, ms, k, nact, is_keep, tgt, feasible[t]); rewards.append(r)
    ...
    torch.save({"state_dict": online.state_dict(), "dim": dim, "n_act": 7, "controller_type": "Double-DQN",
                "topology_agnostic": True, "bottleneck_ranking": True}, SUB / "final_learned_4of5_iter2_model.pt")
    pd.DataFrame(tlog).to_csv(SUB / "final_learned_4of5_iter2_train_log.csv", index=False)
...
def part_c():
    dim = len(A.AGN_FEAT_NAMES); ck = torch.load(SUB / "final_learned_4of5_iter2_model.pt", map_location="cpu")
    ...
    pc = pd.concat(pcs, ignore_index=True); pc.to_csv(SUB / "final_learned_4of5_iter2_eval_per_cycle.csv", index=False)
    return pc
...
if __name__ == "__main__":
    ...
    s = pd.DataFrame(srows); s.to_csv(SUB / "final_learned_4of5_iter2_summary.csv", index=False)
    pd.DataFrame(wide).to_csv(SUB / "final_learned_4of5_iter2_action_distribution.csv", index=False)
    # FlexDATE table
    fr = []
    ...
    flex = pd.DataFrame(fr); flex.to_csv(SUB / "final_learned_4of5_iter2_flexdate_table.csv", index=False)
    # audit
    ...
    json.dump(audit, open(SUB / "FINAL_LEARNED_4OF5_ITER2_AUDIT.json", "w"), indent=2)
    (SUB / "FINAL_LEARNED_4OF5_ITER2_AUDIT.md").write_text("# Final Learned 4/5 — Audit\n\n" +
    ...
    (SUB / "FINAL_LEARNED_4OF5_ITER2_VERDICT.md").write_text("\n".join(V))
